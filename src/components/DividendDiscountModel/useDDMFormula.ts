export const useDDMFormula = () => {
  ...
  const formula = (dividendLast12Months: number, dividendAverageAnnualGrowthRate: number, requiredRateOfReturn = 0.08) => {
    ...
    console.log('dividing '+numerator(dividendLast12Months, dividendAverageAnnualGrowthRate) + ' by '+demoninator(dividendAverageAnnualGrowthRate, requiredRateOfReturn))
    return numerator(dividendLast12Months, dividendAverageAnnualGrowthRate) / demoninator(dividendAverageAnnualGrowthRate, requiredRateOfReturn);
  }
  return formula;
};
