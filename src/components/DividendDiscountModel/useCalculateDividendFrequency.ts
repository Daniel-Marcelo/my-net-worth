...
export const useCalculateDividendFrequency = (history: YFDividendHistory.HistoryList[]) => {
    ...
    useEffect(() => {
        const updatedYearToDividendCount: { [key: number]: number } = orderedHistory.reduce((acc, item) => {
            return {
                ...acc,
        ...
        const data: { [key: number]: number } = Object.entries(updatedYearToDividendCount).reduce((acc, [key, value]) => {
            return {
                ...acc,
